...
export type OccurrenceState = "reserved" | "active" | "interrupted" | "completed" | "abandoned";
export type AttemptOutcome = "started" | "superseded" | "cancelled" | "failed" | "completed";
...
export function seriesTrackKey(seriesTitle: string, separateTrack?: string | null): string {
...
const isOrdered = (position: TrackPosition): position is TrackPosition & { season: number; episode: number } =>
...
function isUnambiguousOpener(position: TrackPosition): boolean {
  if (isOrdered(position)) return position.season === 1 && position.episode === 1;
...
function expectedSuccessor(position: TrackPosition): string {
  if (isOrdered(position)) return `S${position.season}E${position.episode + 1} or S${position.season + 1}E1`;
  return `ordinal ${(position.ordinal ?? 0) + 1}`;
}

/** Ordering inside one track: numbered episodes first, then ordinal fallbacks. */
function compareIdentities(left: EpisodeIdentityRecord, right: EpisodeIdentityRecord): number {
...
function coverageOf(
  ...
  if (!clipped.length || end <= start) return { coveredMs: 0, contiguous: false };
  let cursor = start;
  let coveredMs = 0;
  let contiguous = true;
...
export function createAiringLedger(database: MarkTvDatabase) {
  ...
  const read = <T>(sql: string, ...parameters: unknown[]) => {
    const row = database.prepare(sql).get(...parameters) as { json: string } | undefined;
  ...
  const seriesTrack = (trackKey: string) =>
    read<SeriesTrackRecord>("SELECT json FROM airing_series_tracks WHERE track_key = ?", trackKey);
  ...
  const trackHold = (trackKey: string) =>
    read<TrackHold>("SELECT json FROM airing_track_holds WHERE track_key = ?", trackKey);
  ...
  const completionFloor = (trackKey: string) =>
    read<CompletionFloor>("SELECT json FROM airing_completion_floors WHERE track_key = ?", trackKey);
  ...
  const ensureSeriesTrack = database.transaction((input: SeriesTrackInput) => {
    ...
    if (existing) {
      ...
      database
        .prepare("UPDATE airing_series_tracks SET title = ?, updated_at = ?, json = ? WHERE track_key = ?")
        .run(updated.title, updated.updatedAt, JSON.stringify(updated), trackKey);
   ...
   */
  const establishInitialPosition = database.transaction((input: InitialPositionInput) => {
    const at = input.at ?? now();
    const track = seriesTrack(input.trackKey);
    if (!track) return refusal<SeriesTrackRecord>("unknown-track", `no track ${input.trackKey}`);
    ...
    const record: SeriesTrackRecord = { ...track, initialPosition: position, updatedAt: at };
    database
      .prepare("UPDATE airing_series_tracks SET updated_at = ?, json = ? WHERE track_key = ?")
      .run(at, JSON.stringify(record), input.trackKey);
    return accepted(record, false);
  });

  const holdTrack = database.transaction((input: { trackKey: string; reason: TrackHoldReason; detail?: string; occurrenceKey?: string; at?: string }) => {
    const at = input.at ?? now();
    ...
    database
      ...
      .run(record.trackKey, record.reason, record.detail, record.occurrenceKey, record.createdAt, at, JSON.stringify(record));
    return accepted(record, existing === undefined);
  });

  const releaseTrackHold = database.transaction((trackKey: string, at?: string) => {
    void (at ?? now());
    return database.prepare("DELETE FROM airing_track_holds WHERE track_key = ?").run(trackKey).changes > 0;
  });

  const ensureEpisodeIdentity = database.transaction((input: EpisodeIdentityInput) => {
    const at = input.at ?? now();
    const season = input.season ?? null;
    const episode = input.episode ?? null;
    const ordinal = input.ordinal ?? null;
    const track = seriesTrack(input.trackKey);
    if (!track) return refusal<EpisodeIdentityRecord>("unknown-track", `no track ${input.trackKey}`);
    if ((season === null || episode === null) && ordinal === null) {
      holdTrack({ trackKey: input.trackKey, reason: "missing-position", detail: input.episodeKey, at });
    ...
    if (current) {
      if (current.trackKey !== input.trackKey || current.positionKey !== key) {
        holdTrack({
          trackKey: input.trackKey,
        ...
        return refusal<EpisodeIdentityRecord>("ambiguous-position", `${input.episodeKey} changed position`);
      }
      const updated: EpisodeIdentityRecord = { ...current, title: input.title, updatedAt: at };
      database
        .prepare("UPDATE airing_episode_identities SET title = ?, updated_at = ?, json = ? WHERE episode_key = ?")
        .run(updated.title, updated.updatedAt, JSON.stringify(updated), input.episodeKey);
      return accepted(updated, false);
    }
    const occupant = database
      .prepare("SELECT episode_key FROM airing_episode_identities WHERE track_key = ? AND position_key = ?")
  ...
  const reserveOccurrence = database.transaction((input: OccurrenceReservationInput) => {
    const at = input.at ?? now();
    if (!input.occurrenceKey || Number.isNaN(input.sourceStartMs) || Number.isNaN(input.sourceEndMs)) {
      return refusal<OccurrenceReservation>("invalid-interval", "source interval and key are required");
    }
    if (!Number.isFinite(input.sourceStartMs) || !Number.isFinite(input.sourceEndMs) || input.sourceEndMs <= input.sourceStartMs) {
    ...
    if (existing) {
      ...
      if (!same) {
        return refusal<OccurrenceReservation>("id-conflict", `${input.occurrenceKey} already reserved differently`);
      }
      return accepted(existing, false);
    }
    if (!seriesTrack(input.trackKey)) {
      return refusal<OccurrenceReservation>("unknown-track", `no track ${input.trackKey}`);
    }
    const hold = trackHold(input.trackKey);
    if (hold) return refusal<OccurrenceReservation>("track-held", hold.reason);
    const identity = episodeIdentity(input.episodeKey);
    if (!identity || identity.trackKey !== input.trackKey) {
      return refusal<OccurrenceReservation>("unknown-episode", `${input.episodeKey} is not on ${input.trackKey}`);
    }
  ...
  const setOccurrenceState = (occurrenceKey: string, state: OccurrenceState, at: string) => {
    const current = occurrence(occurrenceKey);
    if (!current) return;
    const updated: OccurrenceReservation = { ...current, state, updatedAt: at };
    database
      .prepare("UPDATE airing_occurrence_reservations SET state = ?, updated_at = ?, json = ? WHERE occurrence_key = ?")
      .run(state, at, JSON.stringify(updated), occurrenceKey);
  };

  const recordAttempt = database.transaction((input: OccurrenceAttemptInput) => {
    const at = input.at ?? now();
    const reservation = occurrence(input.occurrenceKey);
    if (!reservation) {
      return refusal<OccurrenceAttempt>("unknown-occurrence", `no occurrence ${input.occurrenceKey}`);
    }
    ...
    if (existing) {
      ...
      if (!same) return refusal<OccurrenceAttempt>("id-conflict", `${input.attemptId} already recorded`);
      return accepted(existing, false);
    }
    const maxIndex = database
      .prepare("SELECT MAX(attempt_index) AS value FROM airing_occurrence_attempts WHERE occurrence_key = ?")
    ...
    if (record.outcome === "started") setOccurrenceState(record.occurrenceKey, "active", at);
    return accepted(record, true);
  });

  const recordInterval = (
    table: "airing_published_source_intervals" | "airing_aired_source_intervals",
    ...
    const reservation = occurrence(input.occurrenceKey);
    if (!reservation) {
      return refusal<SourceInterval>("unknown-occurrence", `no occurrence ${input.occurrenceKey}`);
    }
    if (!Number.isFinite(input.sourceStartMs) || !Number.isFinite(input.sourceEndMs) || input.sourceEndMs <= input.sourceStartMs) {
    ...
    if (existing) {
      const same =
        ...
        existing.sourceMediaId === (input.sourceMediaId ?? reservation.sourceMediaId);
      if (!same) return refusal<SourceInterval>("id-conflict", `${input.intervalId} already recorded`);
      return accepted(existing, false);
    }
  ...
  const recordPublishedInterval = database.transaction((input: SourceIntervalInput) =>
    recordInterval("airing_published_source_intervals", "published_at", input),
  );
  const recordAiredInterval = database.transaction((input: SourceIntervalInput) =>
  ...
  const beginOccurrence = database.transaction((input: {
    ...
    if (!reservation || reservation.trackKey !== input.trackKey) {
      return refusal<ActiveOccurrence>("unknown-occurrence", `no occurrence ${input.occurrenceKey} on ${input.trackKey}`);
    }
    if (reservation.state === "completed") {
      return refusal<ActiveOccurrence>("already-credited", `${input.occurrenceKey} is completed`);
    }
    if (reservation.state === "abandoned") {
      return refusal<ActiveOccurrence>("id-conflict", `${input.occurrenceKey} was abandoned`);
    }
    const requestedOffset = input.sourceOffsetMs ?? reservation.sourceStartMs;
    if (!Number.isFinite(requestedOffset) || requestedOffset < reservation.sourceStartMs || requestedOffset > reservation.sourceEndMs) {
      return refusal<ActiveOccurrence>("invalid-interval", `source offset ${requestedOffset} is outside ${reservation.sourceStartMs}..${reservation.sourceEndMs}`);
    }
    ...
    if (existing) {
      ...
      const record: ActiveOccurrence = {
        ...
        sourceMediaId: existing.sourceMediaId || input.sourceMediaId || reservation.sourceMediaId,
  ...
  const advanceOccurrenceOffset = database.transaction((input: {
    trackKey: string;
    sourceOffsetMs: number;
    at?: string;
  }) => {
    ...
    if (!active) return refusal<ActiveOccurrence>("no-active-occurrence", `no active occurrence on ${input.trackKey}`);
    const reservation = occurrence(active.occurrenceKey);
    if (!reservation || !Number.isFinite(input.sourceOffsetMs) || input.sourceOffsetMs > reservation.sourceEndMs) {
      return refusal<ActiveOccurrence>("invalid-interval", `source offset ${input.sourceOffsetMs} is outside the active occurrence`);
    }
    ...
    const record: ActiveOccurrence = { ...active, sourceOffsetMs: input.sourceOffsetMs, updatedAt: at };
    writeActive(record, at);
    return accepted(record, false);
  });

  const interruptOccurrence = database.transaction((input: {
    trackKey: string;
    sourceOffsetMs?: number;
    at?: string;
  }) => {
    ...
    if (!active) return refusal<ActiveOccurrence>("no-active-occurrence", `no active occurrence on ${input.trackKey}`);
    const reservation = occurrence(active.occurrenceKey);
    if (!reservation || (input.sourceOffsetMs !== undefined && (!Number.isFinite(input.sourceOffsetMs) || input.sourceOffsetMs > reservation.sourceEndMs))) {
      return refusal<ActiveOccurrence>("invalid-interval", `source offset ${input.sourceOffsetMs} is outside the active occurrence`);
    }
    if (input.sourceOffsetMs !== undefined && input.sourceOffsetMs < active.sourceOffsetMs) {
      return refusal<ActiveOccurrence>(
        "offset-regression",
        `${input.sourceOffsetMs} < ${active.sourceOffsetMs}`,
      );
  ...
  const resumeActiveOccurrence = database.transaction((input: { trackKey: string; at?: string }) => {
    const at = input.at ?? now();
    const active = activeOccurrence(input.trackKey);
    if (!active) return refusal<ActiveOccurrence>("no-active-occurrence", `no active occurrence on ${input.trackKey}`);
    if (active.state === "active") return accepted(active, false);
    const record: ActiveOccurrence = { ...active, state: "active", resumedAt: at, updatedAt: at };
    writeActive(record, at);
    setOccurrenceState(record.occurrenceKey, "active", at);
    return accepted(record, false);
  });
   ...
   */
  const clearActiveOccurrence = database.transaction((input: {
    trackKey: string;
    occurrenceKey?: string;
    at?: string;
  }) => {
    ...
    database.prepare("DELETE FROM airing_active_occurrences WHERE track_key = ?").run(input.trackKey);
    setOccurrenceState(active.occurrenceKey, "abandoned", at);
    return accepted(active, false);
  });

  const evaluateOccurrence = (occurrenceKey: string): OccurrenceEvaluation | undefined => {
    ...
    return {
      ...
      complete: publishedCoverage.contiguous && explicitAiredInterval && airedCoverage.contiguous,
    };
  };

  const completeOccurrence = database.transaction((input: { occurrenceKey: string; at?: string }) => {
    const at = input.at ?? now();
    const reservation = occurrence(input.occurrenceKey);
    if (!reservation) {
      return refusal<CompletionCredit>("unknown-occurrence", `no occurrence ${input.occurrenceKey}`);
    }
    const evaluation = evaluateOccurrence(input.occurrenceKey)!;
    if (!evaluation.complete) {
      const gaps = [
        evaluation.contiguousPublished ? undefined : "published coverage is not contiguous",
        evaluation.explicitAiredInterval ? undefined : "no explicit aired interval",
        evaluation.contiguousAired ? undefined : "aired coverage is not contiguous",
      ].filter((item): item is string => item !== undefined);
      return refusal<CompletionCredit>("insufficient-evidence", gaps.join("; "));
    }
    ...
    if (!identity || identity.trackKey !== reservation.trackKey) {
      return refusal<CompletionCredit>("unknown-episode", `${reservation.episodeKey} is not registered`);
    }
    ...
    if (floor) {
      ...
      const initial = seriesTrack(reservation.trackKey)?.initialPosition ?? null;
      if (initial) {
    ...
    if (active?.occurrenceKey === reservation.occurrenceKey) {
      database.prepare("DELETE FROM airing_active_occurrences WHERE track_key = ?").run(reservation.trackKey);
    }
