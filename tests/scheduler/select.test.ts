...
const select = (pool: Pool, items: MediaItem[], kind: "episode" | "movie") =>
  selectCandidate({
    pool,
    items,
    kind,
...
test("does not select an episode from a movie slot or a mismatched pool", () => {
  ...
  expect(
    select(moviePool, [item("episode", "episode")], "movie"),
  ).toBeUndefined();
});
...
test("advances episodes in season and episode order after the last chronological play", () => {
  const episodes = [3, 1, 2].map((episode) => ({
  ...
  const second = selectCandidate({
    ...
    items: episodes,
    kind: "episode",
    history: [{ mediaId: first.item!.id, at: "2026-09-18T19:00:00.000Z" }],
    at: "2026-09-18T20:00:00.000Z",
    seed: "selection",
  });
...
test("shuffle selection is stable for the same seed regardless of input item order", () => {
  ...
  const reversed = selectCandidate({
    pool,
    items: [...items].reverse(),
    kind: "episode",
    history: [],
...
test("orders season 1 before season 2 when a show title only differs by case, spacing, and a trailing year", () => {
  ...
  const episodes: MediaItem[] = [
    {
      ...item("episode-s2e1", "episode"),
      showTitle: "show",
      season: 2,
      episode: 1,
    },
    {
      ...item("episode-s1e1", "episode"),
      showTitle: "Show (2019)",
      season: 1,
      episode: 1,
    },
    {
      ...item("episode-s1e2", "episode"),
      showTitle: "Show",
      season: 1,
      episode: 2,
    },
  ...
  for (let slot = 0; slot < episodes.length; slot += 1) {
    const history = playedIds.map((mediaId, index) => ({
      mediaId,
      at: `2026-09-18T19:0${index}:00.000Z`,
    }));
...
test("enforces cooldown unless relaxation is explicitly enabled", () => {
  ...
  const input = {
    ...
    items: [movie],
    kind: "movie" as const,
    history: [{ mediaId: movie.id, at: "2026-09-18T19:00:00.000Z" }],
    at: "2026-09-18T20:00:00.000Z",
    seed: "selection",
  };
  expect(selectCandidate(input)).toEqual({ item: undefined, relaxed: false });
  expect(selectCandidate({ ...input, allowCooldownRelaxation: true })).toEqual({
    item: movie,
    relaxed: true,
  });
});
...
test("episode cooldown relaxation cannot replay a completed episode", () => {
  ...
  expect(
    selectCandidate({
      ...
      items: [episode],
      kind: "episode",
      history: [{ mediaId: episode.id, at: "2026-09-18T19:00:00.000Z" }],
      at: AT,
      seed: "selection",
      allowCooldownRelaxation: true,
    }),
...
test("another encode of a completed episode cannot replay the same season and episode", () => {
  ...
  expect(
    selectCandidate({
      pool,
      items: [original, alternate, second],
      kind: "episode",
      history: [{ mediaId: original.id, at: "2026-09-18T19:00:00.000Z" }],
      at: AT,
      seed: "selection",
    }).item?.id,
  ).toBe(second.id);
...
test("unknown episode position holds that series instead of guessing a successor", () => {
  ...
  expect(
    selectCandidate({
      pool,
      items: [unknown, numbered],
      kind: "episode",
      history: [{ mediaId: unknown.id, at: "2026-09-18T19:00:00.000Z" }],
      at: AT,
      seed: "selection",
    }).item,
  ).toBeUndefined();
...
const AT = "2026-09-18T20:00:00.000Z";
const historyAt = (mediaIds: string[]) =>
...
test("EP03 does not wrap after the last episode of a series (F02)", () => {
  ...
  const pool = episodePool(
    "f02",
    episodes.map(({ id }) => id),
  );
  const exhausted = selectCandidate({
    pool,
    items: episodes,
    kind: "episode",
    history: historyAt(["f02-e1", "f02-e2", "f02-e3"]),
    at: AT,
    seed: "selection",
  });
...
test("EP03 falls back to another series instead of wrapping the exhausted one", () => {
  ...
  const selection = selectCandidate({
    ...
    items,
    kind: "episode",
    history: historyAt(["alpha-e1", "alpha-e2"]),
    at: AT,
    seed: "selection",
  });
...
test("EP04 does not skip over a missing or unavailable next episode (F02)", () => {
  const missing = [
    ep("f02-e1", "F02 Show", 1, 1),
    ep("f02-e3", "F02 Show", 1, 3),
  ];
  const missingPool = episodePool(
    "f02-missing",
    missing.map(({ id }) => id),
  );
  expect(
  ...
  const unavailable = [
    ep("f02-e1", "F02 Show", 1, 1),
    ep("f02-e2", "F02 Show", 1, 2, false),
    ep("f02-e3", "F02 Show", 1, 3),
  ];
  const unavailablePool = episodePool(
    "f02-unavailable",
    unavailable.map(({ id }) => id),
  );
  expect(
    selectCandidate({
...
test("EP05 does not select an earlier newly discovered episode below the history floor", () => {
  ...
  const forward = selectCandidate({
    ...
    items: episodes,
    kind: "episode",
    history: historyAt(["floor-e2", "floor-e3"]),
    at: AT,
    seed: "selection",
  });
  ...
  const exhausted = selectCandidate({
    ...
    items: episodes,
    kind: "episode",
    history: historyAt(["floor-e2", "floor-e3", "floor-e4"]),
    at: AT,
    seed: "selection",
  });
  expect(exhausted.item).toBeUndefined();
...
test("[EP] a series that has started again keeps advancing through the new run", () => {
  const episodes = [1, 2, 3].map((n) => ep(`run-e${n}`, "Run Show", 1, n));
  const pool = episodePool(
    "run",
    episodes.map(({ id }) => id),
  );
  const wrapAllowed = new Set(["run show"]);
  // The run reached S1E3 - the last episode - and starts again at S1E1.
  const first = selectCandidate({
    pool,
    items: episodes,
    kind: "episode",
    history: historyAt(["run-e1", "run-e2", "run-e3"]),
    wrapAllowed,
    at: AT,
    seed: "selection",
  });
  expect(first.item?.id).toBe("run-e1");
  // The next selection must CONTINUE the new run. The highest episode ever
  // played is still S1E3, so reading the floor as "highest episode" wraps to
  // S1E1 again and the same file fills the channel twice in a day.
  const second = selectCandidate({
    pool,
    items: episodes,
    kind: "episode",
    history: historyAt(["run-e1", "run-e2", "run-e3", "run-e1"]),
    wrapAllowed,
    at: AT,
    seed: "selection",
  });
  expect(second.item?.id).toBe("run-e2");
  const third = selectCandidate({
    pool,
    items: episodes,
    kind: "episode",
    history: historyAt(["run-e1", "run-e2", "run-e3", "run-e1", "run-e2"]),
    wrapAllowed,
    at: AT,
    seed: "selection",
  });
  expect(third.item?.id).toBe("run-e3");
});

test("[EP] a history that steps back without finishing the run is not a restart", () => {
  // An out-of-order regeneration leaves S1E2 played on one date and S1E1 on a
  // later one. The run never reached S1E4, so this is a torn history, not a
  // wrap: the floor must stay at S1E2 and carry on to S1E3 rather than start
  // the series over.
  const episodes = [1, 2, 3, 4].map((n) => ep(`torn-e${n}`, "Torn Show", 1, n));
  const pool = episodePool(
    "torn",
    episodes.map(({ id }) => id),
  );
  const selection = selectCandidate({
    ...
    items: episodes,
    kind: "episode",
    history: historyAt(["torn-e2", "torn-e1"]),
    wrapAllowed: new Set(["torn show"]),
    at: AT,
    seed: "selection",
  });
  expect(selection.item?.id).toBe("torn-e3");
});

test("EP06 crosses seasons deterministically and stops when the season opener is missing (F17)", () => {
  const season = [
    ep("f17-s1e1", "F17 Show", 1, 1),
    ep("f17-s1e2", "F17 Show", 1, 2),
    ep("f17-s2e1", "F17 Show", 2, 1),
    ep("f17-s2e2", "F17 Show", 2, 2),
  ];
  ...
  expect(
    selectCandidate({
      ...
      items: shuffled,
      kind: "episode",
      history: historyAt(["f17-s1e2"]),
      at: AT,
      seed: "selection",
    }).item?.id,
  ).toBe("f17-s2e1");
  const gapItems = [season[0], season[1], season[3]];
  const gapPool = episodePool(
    "f17-gap",
    gapItems.map(({ id }) => id),
  );
  expect(
